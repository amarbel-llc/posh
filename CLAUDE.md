...
- **Versioning (eng-versioning(7)):** `version.env` (`POSH_VERSION`) at the
  repo root is the single source of truth for the `posh` crate. `flake.nix`
  reads it; `crates/posh/build.rs` is a drift guard that FAILS the build if
  `Cargo.toml`'s `package.version` disagrees. Bump all sites together with
  `just bump-version <sem>` (rewrites version.env + Cargo.toml + Cargo.lock);
  ...
  carries its own lineage (it's meant to be a separable tool).
