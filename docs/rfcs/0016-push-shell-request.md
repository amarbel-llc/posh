...
# Push Shell Request (`PUSH_SHELL`, `PUSH_SHELL_REQUEST`)

## Abstract

A viewport asks its session's daemon to create a new anonymous session in the
session's working directory and move the viewport onto it — FDR 0020's *push
shell*. This document specifies the two capability entries that carry the
exchange: `PUSH_SHELL` (id 21), an offer negotiated as a request/answer pair,
and `PUSH_SHELL_REQUEST` (id 22), a request carrying a token that makes a
repeated request harmless even after the viewport has moved. The reply is the
existing RFC 0008 §3.1 re-home; nothing new is specified for it.
...
### 2. `PUSH_SHELL` (id 21): the offer

Payload: empty in both directions. A non-empty payload MUST be treated as
absent.

- A viewport that can perform a push shell (§5) MUST send `PUSH_SHELL` in its
...
- A daemon that implements this document, on receiving `PUSH_SHELL` from a
  connection, MUST send `PUSH_SHELL` to that connection **once**, on the first
  ...
  that did not ask.
- A viewport MUST NOT offer push shell to its user unless it has received
  `PUSH_SHELL` during the current attach.
- The **bridge** MUST forward a viewport's `PUSH_SHELL` to the daemon (as a
  `Tag::ClientCaps` table entry, like the RFC 0014 §3 entries).
- The **relay MUST NOT** forward it. Its viewport is then never offered push
  shell.

### 3. `PUSH_SHELL_REQUEST` (id 22): the request

Payload: exactly 8 bytes, a big-endian unsigned 64-bit **token**. Token `0`
is reserved; an entry with any other length, or token `0`, MUST be ignored.

- A viewport performs a push shell by choosing a token uniformly at random
  from the nonzero u64 range and sending a `PUSH_SHELL_REQUEST` carrying it.
...
A daemon that receives a `PUSH_SHELL_REQUEST` MUST:
...
2. **Create** a new session in its own group:
   ...
   - working directory: the session shell's last OSC-7 directory, else the
     daemon's own working directory, else `$HOME`;
...
- A viewport that was not offered push shell MUST fall back to FDR 0008's
...
A push shell runs `$POSH_ESCAPE_CMD` with the daemon's privileges, exactly as
...
This document requests capability ids **21** (`PUSH_SHELL`) and **22**
(`PUSH_SHELL_REQUEST`). RFC 0001's registry gains their rows, in place, in the
...
- FDR 0008 — the overlay this falls back to. FDR 0016 — the stack.
  FDR 0020 — the feature. ADR 0007 — why the relay is excluded.
