...
| `session.switch` | `{"target": <string>, "previous"?: <string>}` | Switch the viewport to the named session (an RFC 0001 target: `[user@]host:[group/]session`, `:[group/]session`, or a bare local name) — FDR 0016. The client ends its attach with a switch outcome and its front door re-attaches to `target`. `{"target": "<host>:+"}` (or `":+"`) creates a new auto-id session there first. `previous` says what becomes of the session the viewport is LEAVING: `"keep"` (default — it stays running detached), `"kill"` (killed once the new attach is established, unless other viewports are still attached to it, in which case it is kept and the client is told), or `"force-kill"` (killed regardless, throwing other viewports out). A selection made from inside a session WITHOUT `previous` re-shows the renderer as a `palette` view asking the question, whose commands re-issue `session.switch` with `previous` set. Invalid target or `previous` → `-32602`. |

| `session.pop`    | `{"previous"?: <string>}`               | *Back* (FDR 0016, stacked switching): switch the viewport to the session it most recently switched AWAY from and kept — the top of the front door's session stack. The target is never named by the renderer (the client resolves it from the stack); `previous` has `session.switch`'s meaning for the session being left. Without `previous` from inside a session the client re-shows the renderer with the leave question, whose commands re-issue `session.pop` with `previous`. With an empty stack the client reports "nothing to go back to" and stays. The palette's *Back to …* command carries this action and is offered only while the stack has a top. |
| `session.leave`  | `{"previous": <string>}`                | The leave prompt's answer (FDR 0016, *Leaving posh*): what becomes of the anonymous sessions the viewport created, now that it is leaving posh with no switch to follow. `previous` has `session.switch`'s meaning applied to EVERY candidate — `"keep"` (they stay running), `"kill"` (each killed unless other viewports are attached to it), `"force-kill"` (killed regardless). Unlike the other session methods this is issued by a STANDALONE chooser — the front door hosts the renderer with no session attached, after the last attach has returned — so there is no target to name and no view to return to; the client acts and ends. `ui.cancelled` from this prompt is a keep. Missing or invalid `previous` → `-32602`. |
