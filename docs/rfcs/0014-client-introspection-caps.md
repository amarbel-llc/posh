...
and which cascade step said so: `<src>` is one of `kernel`, `osc7`, `start`,
`home` (`caller` is reserved: a daemon has no caller cwd). A writer that does not run the cascade omits both (the
...
| field | value |
|---|---|
| `kind` | `palette` (the Commands palette, or a dialog it hosts), `picker` (the session picker, in-session or the standalone `ph` chooser), or `notice` (the must-dismiss pop modal, RFC 0005 §3.6) |
