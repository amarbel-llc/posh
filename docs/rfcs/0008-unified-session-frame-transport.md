...
provide a single switch (`POSH_FRAMESYNC`-style build/env flag) that forces the
daemon to emit `Tag::Output` only and the bootstrap to use §5.2(2) — restoring
Architecture A in one step.
