> **Note:** this directory is the original C++ **mosh** tree, kept as the
> porting reference for **posh** — the Rust rewrite combining mosh with zmx
> (terminal session persistence) that lives at the [repo root](../README.md).
> Build it with `nix build .#mosh` or `just zz-mosh/build-autotools`; the
> upstream GitHub workflows under `.github/` here are intentionally inert.
