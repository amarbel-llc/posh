...
protobuf_LIBS=$(
  l=libprotobuf.a
  for i in /opt/local/lib /usr/local/lib; do if [ -f $i/$l ]; then echo $i/$l; fi; done
)
if [ -z "$protobuf_LIBS" ]; then
  echo "Can't find libprotobuf.a"
  exit 1
fi
export protobuf_LIBS
if ! pkg-config --cflags protobuf >/dev/null 2>&1; then
  protobuf_CFLAGS=-I$(for i in /opt /usr; do
    d=$i/local/include
    if [ -d $d/google/protobuf ]; then echo $d; fi
  done)
  if [ "$protobuf_CFLAGS" = "-I" ]; then
    echo "Can't find protobuf includes"
    exit 1
  fi
