...
pub enum Choice {
    ...
    Action { method: String, params: Value },
    /// The user dismissed the view (Esc, the renderer gone, stdin closed).
    Cancelled,
    /// The renderer does not draw the requested view (an older binary).
    Unsupported,
}
...
/// because the loop paints at the same geometry the renderer was spawned at.
fn run_standalone(first_show: impl FnOnce(&mut Palette)) -> util::Result<Choice> {
