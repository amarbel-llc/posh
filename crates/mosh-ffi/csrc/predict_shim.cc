...
{
  return static_cast<uint16_t>( g_clock_ms );
}
...
// Process-global injected clock (timestamp() is a free function, not per-handle).
void mosh_clock_set( uint64_t ms )
{
  g_clock_ms = ms;
}

void* mosh_predict_new( int width, int height, int display_pref, int predict_overwrite )
{
...
void mosh_predict_free( void* h )
{
  delete static_cast<MoshPredict*>( h );
}
